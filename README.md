...
Tuned for the [Anima](https://github.com/sorryhyun/anima_lora) DiT — its modulation guidance, DCW calibrator artifacts, and per-block guidance presets are derived from the Anima training/inference pipeline. See the [anima_lora repo](https://github.com/sorryhyun/anima_lora) for the underlying methods documentation (`docs/methods/dcw.md`, `docs/inference/mod-guidance.md`, `docs/methods/smc_cfg.md`, `docs/methods/spectrum.md`).
...
| `mod_w_profile` (simple) | `step_i8_skip27` | Per-block guidance preset. `off` disables modulation guidance entirely (no adapter download, no extra hook). `step_i8_skip27` (default, best quality) protects source blocks 0–7 + 27 and applies `w=3` to source blocks 8–26. `step_i14` is the safe option when a LoRA shows anatomy drift. On verified depth-expanded Anima models, these step profiles follow the preserved 28-block source lineage; `uniform_w3` still applies to every live block and recovers pre-0413 legacy behavior. |
| `mod_w` (advanced) | 3.0 | Peak guidance strength applied per-block |
| `mod_start_layer` (advanced) | 8 | First live-model block (inclusive) that receives the steering delta. `0` = uniform legacy behavior |
| `mod_end_layer` (advanced) | -1 | Last live-model block + 1 (exclusive). `-1` = all remaining blocks. On the original 28-block Anima, set to `27` to skip its compensation block |
...
Per-block guidance schedules address quality drift on LoRAs whose distribution sits far from the positive-prompt axis (e.g. early blocks blowing out tonal DC into uniform color collapse). On the original 28-block model, the default `step_i8_skip27` protects source blocks 0–7 and the final compensation block 27 from the steering delta while keeping the base text projection uniform across all blocks. On ANIMA 2.9B's 40-block depth expansion, Spectrum uses the model's verified [`expand_manifest.json`](https://huggingface.co/Gazingstars123/Anima-2.9B/blob/main/expand_manifest.json) lineage to move that schedule onto the 28 preserved source blocks; the 12 newly inserted blocks receive no direct steering delta. Unknown layouts retain the previous live-index behavior instead of guessing a proportional mapping. Advanced `mod_start_layer` / `mod_end_layer` values also remain live-model indices. See [`docs/inference/mod-guidance.md`](https://github.com/sorryhyun/anima_lora/blob/main/docs/inference/mod-guidance.md) in anima_lora for the underlying rationale.
